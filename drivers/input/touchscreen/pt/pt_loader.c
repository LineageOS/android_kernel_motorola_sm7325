...
#if defined(CONFIG_TOUCHSCREEN_PARADE_PLATFORM_FW_UPGRADE) || defined(CONFIG_TOUCHSCREEN_PARADE_BINARY_FW_UPGRADE)
...
#if defined(CONFIG_TOUCHSCREEN_PARADE_PLATFORM_TTCONFIG_UPGRADE) || defined(CONFIG_TOUCHSCREEN_PARADE_MANUAL_TTCONFIG_UPGRADE)
...
static void pt_calibrate_idacs(struct work_struct *calibration_work)
{
	...
	} else if (dut_gen == DUT_PIP2_CAPABLE) {
		pm_stay_awake(dev);
		while (1) {
			pt_debug(dev, DL_WARN, "%s: PIP2 Calibration Start\n", __func__);
			/* Calibrate IDAC for mutual-cap */
			rc = cmd->nonhid_cmd->calibrate_idacs(dev, 0, 0, &status);
			if (rc < 0) {
				pt_debug(dev, DL_ERROR,"%s: calibrate mut-cap error, rc = %d\n", __func__, rc);
				break;
			}
			if (status != 0) {
				pt_debug(dev, DL_ERROR,"%s: calibrate mut-cap status error, sts = %d\n", __func__, status);
			}
			/* initialize mutual-cap baseline */
			rc = cmd->nonhid_cmd->initialize_baselines(dev, 0, 1, &status);
			if (rc < 0) {
				pt_debug(dev, DL_ERROR,"%s: initialize baselines mut-cap error, rc = %d\n", __func__, rc);
				break;
			}
			if (status != 0) {
				pt_debug(dev, DL_ERROR,"%s: initialize baselines mut-cap status error, rc = %d\n", __func__, status);
			}
			/* Calibrate IDAC for self-cap */
			rc = cmd->nonhid_cmd->calibrate_idacs(dev, 0, 2, &status);
			if (rc < 0) {
				pt_debug(dev, DL_ERROR,"%s: calibrate self-cap error, rc = %d\n", __func__, rc);
				break;
			}
			if (status != 0) {
				pt_debug(dev, DL_ERROR,"%s: calibrate self-cap status error, sts = %d\n", __func__, status);
			}
			/* initialize self-cap baseline */
			rc = cmd->nonhid_cmd->initialize_baselines(dev, 0, 4, &status);
			if (rc < 0) {
				pt_debug(dev, DL_ERROR,"%s: initialize baselines self-cap error, rc = %d\n", __func__, rc);
				break;
			}
			if (status != 0) {
				pt_debug(dev, DL_ERROR,"%s: initialize baselines self-cap status error, rc = %d\n", __func__, status);
			}
			break;
		}
		pm_relax(dev);
...
//	u8 data[3 + row_image->row_size];
	...
	data = kzalloc(sizeof(u8) * length, GFP_KERNEL);
	if (data == NULL) {
		return -ENOMEM;
	}
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	while (!cd->cypsoc_picoleaf_data && (0 <= retry--))
		msleep(20);
	#endif
	...
	cmd->request_stop_wd(dev);
	disable_irq(cd->irq);
	(void) cypsoc_picoleaf_firmware_update(cd->cypsoc_picoleaf_data);
	enable_irq(cd->irq);
	cmd->request_start_wd(dev);
#endif

	if (cd->cpdata->disable_update) {
		pt_debug(dev, DL_WARN,
			"%s: Skip Touch FW Update\n", __func__);
		return;
	}
...
static int pt_pip2_upgrade_firmware_from_builtin(struct device *dev)
{
	...
	/* Load default FW name(tt_fw.bin) */
	filename[file_count] = kzalloc(sizeof(PT_FW_FILE_NAME), GFP_KERNEL);
	memcpy(filename[file_count++], PT_FW_FILE_NAME, sizeof(PT_FW_FILE_NAME));
...
static int pt_loader_probe(struct device *dev, void **data)
{
	...
	*data = ld;
	...
	ld->fw_workqueue = create_singlethread_workqueue("pt_fw_workqueue");
	if (ld->fw_workqueue) {
		queue_work(ld->fw_workqueue, &ld->fw_and_config_upgrade);
	} else {
		pt_debug(dev, DL_ERROR, "%s: Create Single WQ Error\n", __func__);
	}
...
static void pt_loader_release(struct device *dev, void *data)
{
	...
	if (ld->fw_workqueue) {
		flush_workqueue(ld->fw_workqueue);
		destroy_workqueue(ld->fw_workqueue);
		ld->fw_workqueue = NULL;
	}
#if PT_FW_UPGRADE
