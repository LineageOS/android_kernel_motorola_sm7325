...
#define PT_PANEL_WAIT_RETRY 100
static int panel_retry_cnt = 0;
int pt_drm_panel_check(struct device_node *dt)
{
	...
	if (dt) {
		if (check_dt(dt)) {
			if (!check_default_tp(dt, "qcom,i2c-touch-active")) {
				msleep(100);
				if (panel_retry_cnt++ < PT_PANEL_WAIT_RETRY) {
					retval = -EPROBE_DEFER;
				} else {
					retval = -ENODEV;
				}
...
EXPORT_SYMBOL_GPL(pt_drm_panel_check);
#endif
#endif
		...
		struct device_node *core_node)
{
	...
	/* Required fields */
	value = of_get_named_gpio_flags(core_node, "parade,irq_gpio", 0, NULL);
	if (value >= 0)
	 ...
	 */
	gpio_value = of_get_named_gpio_flags(core_node, "parade,rst_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->rst_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "parade,ddi_rst_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->ddi_rst_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "parade,vddi_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->vddi_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "parade,vcc_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->vcc_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "parade,avdd_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->avdd_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "parade,avee_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->avee_gpio = gpio_value;

	// Picoleaf GPIOs //
	gpio_value = of_get_named_gpio_flags(core_node, "pico_micon,rst_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->pico_rst_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "pico_micon,vdd_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->pico_vdd_gpio = gpio_value;

	gpio_value = of_get_named_gpio_flags(core_node, "pico_micon,vref_gpio", 0, NULL);
	if (gpio_value >= 0)
		pdata->pico_vref_gpio = gpio_value;

	rc = of_property_read_u32(core_node, "dvdd_voltage", &value);
	if (!rc)
		pdata->dvdd_voltage = value;

	rc = of_property_read_u32(core_node, "avdd_voltage", &value);
	if (!rc)
		pdata->avdd_voltage = value;

	rc = of_property_read_u32(core_node, "dvdd_uA", &value);
	if (!rc)
		pdata->dvdd_uA = value;

	rc = of_property_read_u32(core_node, "avdd_uA", &value);
	if (!rc)
		pdata->avdd_uA = value;
	...
	if (of_property_read_bool(core_node, "parade,update_disable"))
		pdata->disable_update = true;

	pr_debug("%s: irq_gpio:%d rst_gpio:%d\n pico_rst_gpio:%d"
		"hid_desc_reg:%d level_irq_udelay:%d vendor_id:%d prod_id:%d\n"
		"flags:%d easy_wakeup_gesture:%d\n", __func__,
		pdata->irq_gpio, pdata->rst_gpio, pdata->pico_rst_gpio,
