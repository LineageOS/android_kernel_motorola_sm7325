...
struct pt_device_access_factory_data {
	ssize_t pr_buf_len;
	u8 pr_buf[TTHE_TUNER_MAX_BUF];
};
...
struct pt_device_access_data {
	...
	/* OEM Factory Interface  */
	struct class *oem_factory_class;
	struct device *oem_factory_dev;
	struct pt_device_access_factory_data *factory_data;
};
...
/* OEM Factory Interface */
static ssize_t pt_device_access_panel_scan_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad = dev_get_drvdata(dev);
	struct pt_device_access_factory_data *work_data = dad->factory_data;
	int status = STATUS_FAIL;
	u8 config;
	u16 num_elem_read;
	int length = 0;
	...
	mutex_lock(&dad->debugfs_lock);
	memset(work_data, 0, sizeof(struct pt_device_access_factory_data));
	buf_out = work_data->pr_buf;
	if (!buf_out)
		goto release_mutex;

	pm_runtime_get_sync(dad->dev);

	rc = cmd->request_exclusive(dad->dev, PT_REQUEST_EXCLUSIVE_TIMEOUT);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
			"%s: Error on request exclusive rc = %d\n",
			__func__, rc);
	...
	rc = pt_exec_scan_cmd_(dad->dev, dad->panel_scan_type_id);
	if (rc < 0) {
		pt_debug(dad->dev, DL_ERROR,
				"%s: Error on execute panel scan rc = %d\n",
				__func__, rc);
	...
	/* Set length to max to read all */
	rc = pt_ret_scan_data_cmd_(dad->dev, 0, 0xFFFF,
			dad->panel_scan_retrieve_id, dad->ic_buf, &config,
			&num_elem_read, NULL);
	if (rc < 0) {
		pt_debug(dad->dev, DL_ERROR,
	...
	elem_offset = num_elem_read;
	while (num_elem_read > 0) {
		rc = pt_ret_scan_data_cmd_(dad->dev, elem_offset, 0xFFFF,
				dad->panel_scan_retrieve_id, NULL, &config,
				&num_elem_read, buf_offset);
		if (rc < 0)
			goto resume_scan;
...
resume_scan:
	cmd->release_exclusive(dad->dev);

put_pm_runtime:
	pm_runtime_put(dad->dev);

	if (status == STATUS_FAIL)
		length = 0;

	work_data->pr_buf_len += scnprintf(buf_out, TTHE_TUNER_MAX_BUF,
		"SCAN_DATA:");

	for (i = 0; i < length; i++)
		work_data->pr_buf_len += scnprintf(buf_out + work_data->pr_buf_len,
			TTHE_TUNER_MAX_BUF - work_data->pr_buf_len,
			"%02X ", dad->ic_buf[i]);
	work_data->pr_buf_len += scnprintf(buf_out + work_data->pr_buf_len,
			TTHE_TUNER_MAX_BUF - work_data->pr_buf_len,
			":(%d bytes)\n", length);
...
release_mutex:
	if (work_data->pr_buf_len > PAGE_SIZE) {
		pt_debug(dad->dev, DL_ERROR, "%s result data over page size\n", __func__);
		mutex_unlock(&dad->debugfs_lock);
		return -ENOMEM;
	}
	pt_debug(dad->dev, DL_WARN, "%s result data size:%ld byte\n", __func__, work_data->pr_buf_len);
	memcpy(buf, buf_out, work_data->pr_buf_len);
	mutex_unlock(&dad->debugfs_lock);

	return work_data->pr_buf_len;
}

static ssize_t pt_device_access_panel_scan_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&dad->debugfs_lock);
	memset(work_data, 0, sizeof(struct pt_device_access_factory_data));
	length = cmd->parse_sysfs_input(dad->dev, buf, count,
			input_data, ARRAY_SIZE(input_data));
	switch (length) {
	case 1:
		dad->panel_scan_retrieve_id = input_data[0];
...
static DEVICE_ATTR(panel_scan, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				pt_device_access_panel_scan_show, pt_device_access_panel_scan_store);

static ssize_t pt_device_access_cp_panel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad = dev_get_drvdata(dev);
	struct pt_device_access_factory_data *work_data = dad->factory_data;
	...
	memset(work_data, 0, sizeof(struct pt_device_access_factory_data));
	/* Set length to PIP_CMD_MAX_LENGTH to read all */
	work_data->pr_buf_len = pt_run_and_get_selftest_result(
		dad->dev, PT_CORE_CMD_PROTECTED,
		work_data->pr_buf, sizeof(work_data->pr_buf),
		PT_ST_ID_CP_PANEL, PIP_CMD_MAX_LENGTH,
		PT_ST_GET_RESULTS, PT_ST_PRINT_RESULTS,
		PT_PR_FORMAT_DEFAULT);
	...
	pt_debug(dad->dev, DL_WARN, "%s result data size:%ld byte\n", __func__, work_data->pr_buf_len);
	memcpy(buf, work_data->pr_buf, work_data->pr_buf_len);
	mutex_unlock(&dad->debugfs_lock);

	return work_data->pr_buf_len;
}
static DEVICE_ATTR(cp_panel, S_IRUSR | S_IRGRP, pt_device_access_cp_panel_show, NULL);

static ssize_t pt_device_access_cm_panel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad = dev_get_drvdata(dev);
	struct pt_device_access_factory_data *work_data = dad->factory_data;
...
static DEVICE_ATTR(cm_panel, S_IRUSR | S_IRGRP, pt_device_access_cm_panel_show, NULL);

static ssize_t pt_device_access_auto_shorts_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad = dev_get_drvdata(dev);
	struct pt_device_access_factory_data *work_data = dad->factory_data;
...
static DEVICE_ATTR(auto_shorts, S_IRUSR | S_IRGRP, pt_device_access_auto_shorts_show, NULL);

static ssize_t pt_device_access_get_idac_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad = dev_get_drvdata(dev);
	struct pt_device_access_factory_data *work_data = dad->factory_data;
	...
	rc = pt_suspend_scan_cmd_(dad->dev);
	if (rc < 0) {
		pt_debug(dad->dev, DL_ERROR, "%s: Error on suspend scan rc = %d\n",
			__func__, rc);
		goto release_exclusive;
	}

	rc = pt_get_data_structure_cmd_(dad->dev, 0, PIP_CMD_MAX_LENGTH,
		dad->get_idac_data_id, &cmd_status, &data_format,
		&act_length, &dad->ic_buf[5]);
	if (rc < 0) {
		pt_debug(dad->dev, DL_ERROR,
...
put_pm_runtime:
	...
	work_data->pr_buf_len = prepare_print_buffer(status, dad->ic_buf, length,
		work_data->pr_buf, sizeof(work_data->pr_buf), PT_PR_FORMAT_DEFAULT);
...
static ssize_t pt_device_access_get_idac_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct pt_device_access_data *dad = dev_get_drvdata(dev);
	ssize_t length;
	...
	mutex_lock(&dad->debugfs_lock);
	memset(dad->factory_data, 0, sizeof(struct pt_device_access_factory_data));
	length = cmd->parse_sysfs_input(dad->dev, buf, count,
			input_data, ARRAY_SIZE(input_data));
	if (length != 1) {
		pt_debug(dad->dev, DL_ERROR,
...
static DEVICE_ATTR(get_idac, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
				pt_device_access_get_idac_show, pt_device_access_get_idac_store);

static struct attribute *oem_factory_attributes[] = {
	&dev_attr_panel_scan.attr,
	&dev_attr_cp_panel.attr,
	&dev_attr_cm_panel.attr,
	&dev_attr_auto_shorts.attr,
	&dev_attr_get_idac.attr,
	NULL,
};

static struct attribute_group oem_factory_attr_group = {
	.attrs = oem_factory_attributes,
};

static int pt_device_access_factory_sysfs_init(struct pt_device_access_data *dad)
{
	int ret = 0;

	dad->factory_data = kzalloc(sizeof(struct pt_device_access_factory_data), GFP_KERNEL);
	if (dad->factory_data == NULL) {
		ret = -ENOMEM;
		goto err_mem_malloc;
	}
	/* SYSFS */
	dad->oem_factory_class = class_create(THIS_MODULE, "pt_factory");
	if (IS_ERR(dad->oem_factory_class)) {
		ret = PTR_ERR(dad->oem_factory_class);
		goto err_cleate_class;
	}

	dad->oem_factory_dev = device_create(dad->oem_factory_class, NULL, 0, dad, "pt_mfg");
	if (IS_ERR(dad->oem_factory_dev)) {
		pt_debug(dad->dev, DL_DEBUG, "%s: fail - device_create\n", __func__);
		ret = PTR_ERR(dad->oem_factory_dev);
		goto err_cleate_device;
	}

	ret = sysfs_create_group(&dad->oem_factory_dev->kobj, &oem_factory_attr_group);
	if (ret < 0) {
		pt_debug(dad->dev, DL_DEBUG, "%s: fail - sysfs_create_group\n", __func__);
...
err_cleate_sysfs:
	device_destroy(dad->oem_factory_class, 0);
	dad->oem_factory_dev = NULL;
err_cleate_device:
	class_destroy(dad->oem_factory_class);
	dad->oem_factory_class = NULL;
err_cleate_class:
	kfree(dad->factory_data);
err_mem_malloc:
	return ret;
}

static void pt_device_access_factory_sysfs_deinit(struct pt_device_access_data *dad)
{
	if (!dad->oem_factory_class) {
		return;
	}

	sysfs_remove_group(&dad->oem_factory_dev->kobj, &oem_factory_attr_group);
	device_destroy(dad->oem_factory_class, 0);
	dad->oem_factory_dev = NULL;
	class_destroy(dad->oem_factory_class);
	dad->oem_factory_class = NULL;
...
#endif
#endif /* CONFIG_DEBUG_FS */
	rc = pt_device_access_factory_sysfs_init(dad);
	if (rc) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create oem sysfs\n",
			__func__);
		goto create_oem_sysfs;
	}
...
unregister_base_dir:
	pt_device_access_factory_sysfs_deinit(dad);
create_oem_sysfs:
#ifdef CONFIG_DEBUG_FS
